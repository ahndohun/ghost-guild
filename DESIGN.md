...
기질은 더 이상 플레이어가 고르지 않는다. **클래스가 곧 정체성(몸+영혼)**: 각 클래스에 기질(전투 행동 로직)이 내장되고, 클래스별 **특성화 트리**(3티어×2택1, 골드 해금 150/400/900g)가 능력치·스킬뿐 아니라 **행동방식까지** 분화시킨다.

| 클래스 | 내장 기질 (행동 로직 재사용) | 정합 근거 |
|---|---|---|
| Knight | **Vanguard** (신규 중립: b60/g40/f60, 하드룰 없음) | 무난 기준점 — 상황 판단 균형형 |
| Mage | Duelist 로직 (사거리 80~100% 정밀 카이팅, 최고레벨 무기 +15%) | 유리대포의 생존+화력 문법 |
| Priest | Survivor 로직 (위험감지 1.5배·조기 도주, 생존초 ×1.4) | 유지력 = 오래 버티는 운영 |
| Monk | Berserker 로직 (적 200px 안 전리품 무시·도주 없음, 처치 회복+피로) | 맨주먹 근접 몰입 |
| Gambler | Hoarder 로직 (빈사에도 전리품 우회, 골드 +30%) | 도박사의 탐욕 |
...
- `#screen-guild`: name input `player-name`, class-specialization tree cards (temperament cards removed 2026-07-10 v3 — class implies temperament), perk cards `perk-t1-a|perk-t1-b|perk-t2-a|perk-t2-b|perk-t3-a|perk-t3-b`, class cards `class-knight|mage|priest`, upgrade buttons `buy-atk|hp|spd|luck|lvl`, gold display `gold-amount`, buttons `deploy-solo`, `deploy-arena`, `toggle-autorun`. (The trait sliders were replaced by the temperament system on 2026-07-10 — see §2.)
