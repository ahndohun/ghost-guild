...
**특성 노드(Perks) — 정체성을 벼리는 질적 성장 (만능화 금지):** 기질별 3티어 × 2택 1, 골드 해금(150/400/900g), 티어 순차, 기질 교체 시 노드는 기질별로 보존. 모든 노드는 해당 기질을 증폭하는 방향만 — 약점 보완 노드 없음. 예(광전사): T1 '피의 갈증'(처치 회복 +1) vs '전투 본능'(공격속도 +8%), T2 '광란'(HP<35%에서 피해 +25%) vs '철피부'(접촉피해 -20%), T3 '학살자'(엘리트 처치 시 전무기 쿨다운 초기화) vs '불멸의 분노'(치명타 1회 생존). 데이터 테이블로 구현, data-testid `perk-t<tier>-<a|b>` (예: `perk-t1-a`).
