# Colosseum Survivors — Design Spec

> **You don't play the gladiator. You coach them.**
> (display title: **Colosseum Survivors**; repo/URL codename remains ghost-guild)
> An idle management survivors-like: your hero fights, chooses upgrades, and dies **on their own judgment**. You tune who they *are* between runs—then watch your coaching pay off (or backfire) against asynchronous rival builds.

One-line for judges: *An idle colosseum survivors where the gladiator fights on its own judgment—you tune who they are, then pit them against rival builds from around the world.*

**Setting (2026-07-10 확정)**: 검투사 콜로세움. 영웅 = 모래 위에서 처절하게 살아남으려는 검투사(직업은 다양). 아레나 = 관중석에 둘러싸인 투기장. 멀티 상대 = 전 세계 검투사의 저장된 빌드. JRPG 2%는 DQ 클래스 문법 + 다이얼로그로 유지.
...
| Elf Archer | Duelist 로직 | 0.6초 유도 화살 연사와 거리 유지로 싸우는 순수 궁수 |
...
| **Knight** | free | 140 | 85 | Shield Bash | 수호형 | Immovable bulwark / Lowest attack tier |
| **Berserker** | free | 80 | 100 | Whirlwind Axe | 공격 극단 | Blood-fueled whirlwind / Frail and fatigue-bound |
| **Dwarf** | free | 120 | 90 | Earth Shatter | 작은 근접형 | Small hitbox, fast cadence / Short reach |
| **Paladin** | free | 125 | 88 | Holy Smash + weak Heal Pulse (+1HP/5s) | 탱커·힐러 혼합 | Smites and mends / 55% damage, neither peak tank nor healer |
...
| **Warlock** | free | 75 | 95 | Life Drain | 공격 마법사 | 8% damage lifesteal / Fragile and loot-averse |
| **Elf Archer** | free | 90 | 100 | Magic Arrow (0.6s class cadence) | 순수 원거리 궁수 | Rapid homing volleys / 90 HP, weapon slots capped at 3 |
| **Thief** | free | 85 | 115 | Shadow Daggers | 전리품·치명타 | 20% ×2 crit, magnet/gold bonus / Low durability |
...
## 6. Match & multiplayer (asynchronous arena)
...
- Solo run = match of 1. **Arena mode (P2)** = my loadout + up to 3 **rival loadouts** fetched from server pool (async PvP—matchmaking without presence). Server also assigns the seed.
...
- `#screen-title`: logo + `start-game`. Any `?seed` / `?fast` / `?autoplay` key still skips the title.
- First identity gate: an unnamed save opens `name-modal`; the single input keeps `player-name`, confirmation is `confirm-player-name`, optional rename cancellation is `name-modal-close`. Existing named saves skip it. The persistent stage nameplate exposes `rename-player` and `nameplate-best`; there is no always-visible name input or detached Best badge.
- `#screen-guild` has exactly three horizontal growth tabs: `guild-tab-class` (**CLASS & TREE**), `guild-tab-training`, and compatibility testid `guild-tab-gear` (**INVENTORY**). Overview is removed; the lobby stage and identity nameplate stay visible for every tab.
- Class & Tree: roster `class-roster`; cards `class-fighter|knight|berserker|dwarf|paladin|mage|priest|warlock|elf|thief|monk` (`aria-pressed` reflects selection); shared detail `selected-class-detail` plus `selected-class-name|weapons|strength|weakness|behavior`; perk cards `perk-t1-a|perk-t1-b|perk-t2-a|perk-t2-b|perk-t3-a|perk-t3-b|perk-t4-a|perk-t4-b|perk-t5-a|perk-t5-b`. All eleven choices and the detail remain visible without pane scrolling.
- Training: permanent upgrades `buy-atk|hp|spd|luck|lvl`. Inventory: `inventory-panel`, equipped slots `item-slot-relicWeapon|item-slot-armor|item-slot-trinket`, and `stash-list`. Shared balance is `gold-amount`.
- The one dominant lobby action is `battle-open` (**ENTER THE ARENA**), which opens `battle-modal`. Compatibility testids `deploy-solo` and `deploy-arena` live inside that mode chooser; `toggle-autorun` also lives there. Async Arena deployment disables both mode buttons and marks `deploy-arena[aria-busy=true]` until matchmaking settles.
- Settings are behind `settings-open` → `settings-popover`: `sound-toggle` and `coach-replay` (visible label **TUTORIAL**). Coach surfaces remain `coach-panel|coach-run-panel|coach-results-panel` with their existing skip buttons.
- `#screen-run`: `spectator-hud`, canvas frame `#run-canvas-frame`, canvas, in-frame HP/XP `.hud`, and `arena-offline-badge`; hidden mirror `#game-state` carries `data-phase|time|hp|level|kills|gold|seed|mode|class|temperament` and updates every 500ms. Run coach is anchored to the upper canvas zone so the bottom level-up dialog remains clear.
- `#screen-results`: report `result-outcome|score|rank|kills|time`, ledger `result-gold-before|earned|after`, `result-loot`, lists `result-ranking|leaderboard-list`, status `leaderboard-status`, and actions `run-again|adjust-build|back-to-guild`. Offline Arena runs keep the offline status through Results and never submit a world result.
- URL params (dev & E2E): `?seed=N` fixes the seed; `?fast=1` runs without rAF wait; `?autoplay=1` defers its solo start behind the name gate only when the save is unnamed.
